...
if echo "$prompt" | grep -qi "tobefix"; then
  ...
  if echo "$prompt" | grep -qiE "(tobefix.*done|done.*tobefix|finished.*tobefix|tobefix.*finished|completed.*tobefix|tobefix.*completed)"; then
    jq -n '{
      "hookSpecificOutput": {
        ...
        "additionalContext": "TOBEFIX DONE: The user marked a tobefix item as done. Find the item in TOBEFIX.md by ID (F-XXXX) or by matching description. Remove the entire line from the file. Confirm what was removed."
      }
    }'
  ...
  # Check for CHECK/SEARCH pattern (list or search items)
    ...
    jq -n '{
      "hookSpecificOutput": {
        "hookEventName": "UserPromptSubmit",
        "additionalContext": "TOBEFIX CHECK: The user wants to see tobefix items. Read TOBEFIX.md and show relevant items. If they mentioned a topic/keyword, filter to matching items. Otherwise show all items with their IDs and sections."
      }
    }'
  ...
  # Default: ADD new item
    ...
    jq -n '{
      "hookSpecificOutput": {
        "hookEventName": "UserPromptSubmit",
        "additionalContext": "TOBEFIX TRIGGERED: The user mentioned '\''tobefix'\''. Extract the task/issue they described and add it to TOBEFIX.md (in project root). Format: `- [F-XXXX] (YYYY-MM-DD) task text` where XXXX is a random 4-char alphanumeric ID. Add under appropriate section. After adding, briefly confirm what was logged with its ID."
      }
    }'
  fi
