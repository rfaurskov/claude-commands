...
Search the codebase for TODO/TOBEFIX/FIXME/PLACEHOLDER comments not tracked in TOBEFIXED.md:
...
1. Check if it's already in TOBEFIXED.md (by matching description or file reference)
2. If NOT tracked, use AskUserQuestion:
   - "Add this stray item to TOBEFIXED.md?"
   - Show: file:line and the comment text
   - Options: "Yes, add to TOBEFIXED.md" / "No, it's stale - remove from code" / "Skip"
...
For each TOBEFIXED.md item that has a `(file:line)` reference:
...
- Stray comments added to TOBEFIXED.md
